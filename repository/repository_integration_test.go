...
func TestRepository_PostgresEntryGroupByHeadwordUsesExactCanonicalHeadword(t *testing.T) {
	repo, db := newPostgresIntegrationRepository(t)
	seedExactEntryGroupFixture(t, db)

	words, variant, err := repo.GetEntryGroupByHeadword(context.Background(), "to go", false, false, true)
	if err != nil {
		t.Fatalf("GetEntryGroupByHeadword(to go) error = %v", err)
	}
	if variant != nil {
		t.Fatalf("to go returned unexpected variant: %#v", variant)
	}
	assertEntryGroupHeadwords(t, words, "to go")

	words, _, err = repo.GetEntryGroupByHeadword(context.Background(), "togo", false, false, true)
	if err != nil {
		t.Fatalf("GetEntryGroupByHeadword(togo) error = %v", err)
	}
	assertEntryGroupHeadwords(t, words, "togo")

	words, _, err = repo.GetEntryGroupByHeadword(context.Background(), "go off", false, false, true)
	if err != nil {
		t.Fatalf("GetEntryGroupByHeadword(go off) error = %v", err)
	}
	assertEntryGroupHeadwords(t, words, "go off")
	...
	words, variant, err = repo.GetEntryGroupByHeadword(context.Background(), "heads", false, false, true)
	if err != nil {
		t.Fatalf("GetEntryGroupByHeadword(heads) error = %v", err)
	}
	if variant == nil || variant.FormText != "heads" {
		t.Fatalf("variant metadata = %#v, want heads", variant)
	}
	assertEntryGroupHeadwords(t, words, "head")
	if got, want := entryGroupPOS(words), []string{commonmodel.POSNoun, commonmodel.POSVerb}; strings.Join(got, ",") != strings.Join(want, ",") {
		t.Fatalf("variant exact group POS = %v, want %v", got, want)
	}
}
...
func TestRepository_PostgresSearchPhrasesDeduplicatesHeadwords(t *testing.T) {
	repo, db := newPostgresIntegrationRepository(t)
	seedPhraseSearchDedupFixture(t, db)

	phrases, err := repo.SearchPhrases(context.Background(), "to", 2)
	if err != nil {
		t.Fatalf("SearchPhrases(to) error = %v", err)
	}
	assertUniqueSuggestionHeadwords(t, phrases)
	if len(phrases) != 2 {
		t.Fatalf("SearchPhrases(to, limit 2) len = %d, want 2 deduplicated headwords: %#v", len(phrases), phrases)
	}
	if got, want := strings.Join(suggestionHeadwords(phrases), ","), "face-to-face,day-to-day"; got != want {
		t.Fatalf("SearchPhrases(to, limit 2) headwords = %s, want %s", got, want)
	}
	assertSuggestionRepresentative(t, phrases, "face-to-face", 601)
}

func TestRepository_PostgresSearchPhrasesPreloadsTranslationFallback(t *testing.T) {
	repo, db := newPostgresIntegrationRepository(t)
	seedPhraseTranslationFallbackFixture(t, db)

	phrases, err := repo.SearchPhrases(context.Background(), "to", 10)
	if err != nil {
		t.Fatalf("SearchPhrases(to) error = %v", err)
	}
	byHeadword := wordsByHeadword(phrases)

	summary := byHeadword["summary to phrase"]
	if len(summary.SummariesZH) != 1 || summary.SummariesZH[0].SummaryText != "摘要短语" {
		t.Fatalf("summary phrase summaries = %#v, want 摘要短语", summary.SummariesZH)
	}

	school := byHeadword["school to phrase"]
	if len(school.EntryDefinitions) != 1 || school.EntryDefinitions[0].Source != "school" || school.EntryDefinitions[0].TextZHHans != "教材短语" {
		t.Fatalf("school phrase fallback definitions = %#v, want school 教材短语", school.EntryDefinitions)
	}

	sense := byHeadword["sense to phrase"]
	if len(sense.Senses) != 1 || len(sense.Senses[0].GlossesZH) != 1 || sense.Senses[0].GlossesZH[0].TextZHHans != "义项短语" {
		t.Fatalf("sense phrase fallback glosses = %#v, want 义项短语", sense.Senses)
	}
}
...
func seedPhraseSearchDedupFixture(t *testing.T, db *gorm.DB) {
	t.Helper()
	...
	entries := []struct {
		...
		{601, "face-to-face", commonmodel.POSAdjective, 1},
		{602, "face-to-face", commonmodel.POSNoun, 2},
		{603, "face-to-face", commonmodel.POSAdverb, 3},
		{604, "day-to-day", commonmodel.POSAdjective, 10},
		{605, "head-to-head", commonmodel.POSAdjective, 20},
	}
	for _, spec := range entries {
		...
		form := WordVariant{
			ID:              spec.id + 6000,
			WordID:          spec.id,
			FormText:        strings.ReplaceAll(spec.headword, "-", " "),
			NormalizedForm:  norm.NormalizeHeadword(strings.ReplaceAll(spec.headword, "-", " ")),
			RelationKind:    commonmodel.RelationKindAlias,
			SourceRelations: pq.StringArray{"phrase_alias"},
			DisplayOrder:    1,
		}
	...
	if err := migration.RefreshReadModels(db); err != nil {
		t.Fatalf("refresh phrase dedup read models: %v", err)
	}
}

func seedPhraseTranslationFallbackFixture(t *testing.T, db *gorm.DB) {
	t.Helper()
	...
	importRun := commonmodel.ImportRun{
		ID:              610,
		SourceName:      "wiktionary",
		SourcePath:      "/data/phrase-fallback.xml",
		PipelineVersion: "v1",
		Status:          commonmodel.ImportRunStatusCompleted,
		StartedAt:       now,
		FinishedAt:      &now,
	...
	createRows(t, db, &importRun, &schoolRun)

	entries := []Word{
		{ID: 611, Headword: "summary to phrase", NormalizedHeadword: norm.NormalizeHeadword("summary to phrase"), Pos: commonmodel.POSNoun, IsMultiword: true, SourceRunID: importRun.ID},
		{ID: 612, Headword: "school to phrase", NormalizedHeadword: norm.NormalizeHeadword("school to phrase"), Pos: commonmodel.POSNoun, IsMultiword: true, SourceRunID: importRun.ID},
		{ID: 613, Headword: "sense to phrase", NormalizedHeadword: norm.NormalizeHeadword("sense to phrase"), Pos: commonmodel.POSNoun, IsMultiword: true, SourceRunID: importRun.ID},
	}
	for i := range entries {
		createRows(t, db, &entries[i])
		createRows(t, db, &commonmodel.EntryLearningSignal{
			EntryID:       entries[i].ID,
			FrequencyRank: int(entries[i].ID - 610),
			UpdatedAt:     now,
		})
	}
	createRows(t, db, &commonmodel.EntrySummaryZH{
		ID:          6110,
		EntryID:     611,
		Source:      "manual",
		SourceRunID: importRun.ID,
	...
	createRows(t, db, &commonmodel.EntryDefinition{
		ID:                  6120,
		EntryID:             612,
		POS:                 commonmodel.POSNoun,
		Source:              "school",
		SourceRunID:         schoolRun.ID,
		DefinitionOrder:     1,
		TextZHHans:          "教材短语",
		NormalizedZHHansKey: "教材短语",
		UpdatedAt:           now,
	})

	sense := Sense{ID: 6130, WordID: 613, SenseOrder: 1}
	createRows(t, db, &sense)
	createRows(t, db, &commonmodel.SenseGlossZH{
		ID:          6131,
		SenseID:     sense.ID,
		Source:      "manual",
		SourceRunID: importRun.ID,
	...
	if err := migration.RefreshReadModels(db); err != nil {
		t.Fatalf("refresh phrase fallback read models: %v", err)
	}
}

func seedExactEntryGroupFixture(t *testing.T, db *gorm.DB) {
	t.Helper()
	...
	importRun := commonmodel.ImportRun{
		ID:              620,
		SourceName:      "wiktionary",
		SourcePath:      "/data/exact-groups.xml",
		PipelineVersion: "v1",
		Status:          commonmodel.ImportRunStatusCompleted,
		StartedAt:       now,
		FinishedAt:      &now,
	...
	entries := []Word{
		{ID: 621, Headword: "to go", NormalizedHeadword: norm.NormalizeHeadword("to go"), Pos: commonmodel.POSVerb, IsMultiword: true, SourceRunID: importRun.ID},
		{ID: 622, Headword: "to go", NormalizedHeadword: norm.NormalizeHeadword("to go"), Pos: commonmodel.POSNoun, IsMultiword: true, SourceRunID: importRun.ID},
		{ID: 623, Headword: "togo", NormalizedHeadword: norm.NormalizeHeadword("togo"), Pos: commonmodel.POSNoun, SourceRunID: importRun.ID},
		{ID: 624, Headword: "Togo", NormalizedHeadword: norm.NormalizeHeadword("Togo"), Pos: commonmodel.POSNoun, SourceRunID: importRun.ID},
		{ID: 625, Headword: "go off", NormalizedHeadword: norm.NormalizeHeadword("go off"), Pos: commonmodel.POSVerb, IsMultiword: true, SourceRunID: importRun.ID},
		{ID: 626, Headword: "go-off", NormalizedHeadword: norm.NormalizeHeadword("go-off"), Pos: commonmodel.POSNoun, IsMultiword: true, SourceRunID: importRun.ID},
		{ID: 627, Headword: "head", NormalizedHeadword: norm.NormalizeHeadword("head"), Pos: commonmodel.POSNoun, SourceRunID: importRun.ID},
		{ID: 628, Headword: "head", NormalizedHeadword: norm.NormalizeHeadword("head"), Pos: commonmodel.POSVerb, SourceRunID: importRun.ID},
	}
	for i := range entries {
		createRows(t, db, &entries[i])
		createRows(t, db, &commonmodel.EntryLearningSignal{
			EntryID:       entries[i].ID,
			FrequencyRank: int(entries[i].ID - 620),
			UpdatedAt:     now,
		})
	}
	...
	if err := migration.RefreshReadModels(db); err != nil {
		t.Fatalf("refresh exact entry group read models: %v", err)
	}
}
...
func seedHeadwordRelationGroupFixture(t *testing.T, db *gorm.DB) {
	...
	createRows(t, db, &WordVariant{
		...
		WordID:          1002,
		FormText:        "headed",
		NormalizedForm:  norm.NormalizeHeadword("headed"),
		RelationKind:    commonmodel.RelationKindForm,
		FormType:        stringPtr("past"),
		SourceRelations: pq.StringArray{"form_of"},
		DisplayOrder:    2,
...
func assertEntryGroupHeadwords(t *testing.T, words []Word, want string) {
	t.Helper()

	if len(words) == 0 {
		t.Fatalf("entry group is empty, want headword %q", want)
	}
	for _, word := range words {
		if word.Headword != want {
			t.Fatalf("entry group included headword %q in %#v, want only %q", word.Headword, words, want)
		}
	}
}

func wordsByHeadword(words []Word) map[string]Word {
	result := make(map[string]Word, len(words))
	for _, word := range words {
		result[word.Headword] = word
	}
	return result
}
